...
def _fm_date(dt):
    return dt.astimezone(PARIS_TZ).strftime("%d/%m/%Y à %H:%M:%S")  # c'est pour avoir des date à l'heure de paris 
...
def _fetch_meteo():
    sys.path.insert(0, "/opt/ingestion")  # on ajoute dans le path pour pouvoir l'appeler directement  
...
def _on_failure(context:dict):
    ...
    exception = context.get("exception", "inconnu")
    send_email(
        to=[alert_email],
...
def dag_airflow():
    fetch_velib = PythonOperator(
        task_id = "fetch_velib",
        python_callable = _fetch_velib,  # la focntion qu"on va appéleé
